...
// 古い書き方の設定も読めることを確かめます。
func TestStorageEntriesFromLegacy(t *testing.T) {
	cfg := loadConfigFrom(t, `
DefaultWorker: 4
Local:
  name: local
...
`)
	...
	want := map[string]string{"local": "local", "dropbox": "dropbox", "drive": "googledrive"}
	for name, typ := range want {
		if byName[name] != typ {
			t.Errorf("%s の種別 = %q, want %q", name, byName[name], typ)
		}
	}
	...
	for _, e := range entries {
		switch e.Name {
		case "dropbox":
			if got := e.Params.Get("app_key"); got != "KEY" {
				t.Errorf("app_key = %q", got)
			}
		case "drive":
			if got := e.Params.Get("drive_id"); got != "DRIVE" {
				t.Errorf("drive_id = %q", got)
			}
			if got := e.Params.Get("native_files"); got != "skip" {
				t.Errorf("native_files = %q", got)
			}
		}
	}
}

// 両方の書き方を混ぜられることを確かめます。
//
// 既存の設定に新しいストレージを足すとき、全部を書き直さずに
// 済むようにしています。
func TestStorageEntriesMixed(t *testing.T) {
	cfg := loadConfigFrom(t, `
Local:
  name: local
storages:
...
`)
	...
	if len(entries) != 2 {
		t.Fatalf("件数 = %d, want 2（両方の書き方が集まること）", len(entries))
	}
}
